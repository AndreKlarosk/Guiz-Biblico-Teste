    ...
    if (user) {
        ...
        if (becomeModeratorBtn) {
            if (userData.moderador) {
                becomeModeratorBtn.classList.add('hidden');
            } else {
                becomeModeratorBtn.classList.remove('hidden');
            }
        }
        ...
        if (becomeModeratorBtn) becomeModeratorBtn.classList.remove('hidden');
...
async function saveUserToFirestore(user) {
    ...
    try {
        ...
        // Obtenha o documento atualizado para garantir que currentUserData esteja fresco
        const updatedUserDoc = await getDoc(userRef);
        currentUserData = updatedUserDoc.data();
        console.log("currentUserData after saveUserToFirestore:", currentUserData); // Debug log
        console.log("gruposCriados size:", currentUserData.gruposCriados?.length); // Debug log
...
if (createGroupBtn) createGroupBtn.addEventListener('click', async () => {
    ...
    // DEBUG: Log antes de verificar moderação
    console.log("Current currentUserData before moderator check:", currentUserData);
...
if (saveGroupBtn) saveGroupBtn.addEventListener('click', async () => {
    ...
    // Validação de limite de grupos novamente (redundância para segurança no lado do cliente)
    const createdGroupsCount = currentUserData.gruposCriados ? currentUserData.gruposCriados.length : 0;
    const maxGroups = MODERATOR_PLANS[currentUserData.plano]?.grupos;
