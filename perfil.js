...
let profileUid = null; //
let unsubscribeSuggestions = null; // Para desinscrever do listener de sugestões //
...
window.addEventListener('DOMContentLoaded', () => { //
    ...
    onAuthStateChanged(auth, (user) => { //
        ...
        if (currentUser && currentUser.uid === profileUid) { //
            setupNotificationListener(currentUser.uid); // Inicia o listener apenas para o próprio perfil //
        } else {
            // Se não é o próprio perfil, ocultar elementos de usuário logado no header e notificação
    ...
    try {
        ...
        if (userDoc.exists()) { //
            ...
            if (currentUser && currentUser.uid === profileUid) { //
                ...
                // Aplica a borda ao perfil do cabeçalho
                if (profilePhotoContainerHeader) { //
                    profilePhotoContainerHeader.className = 'profile-photo-container'; // Reset classes //
                    if (userDoc.data().bordaEquipada && userDoc.data().bordaEquipada !== 'default') { //
                        profilePhotoContainerHeader.classList.add(userDoc.data().bordaEquipada); //
                    }
                }
            }
        } else {
...
function displayProfileData(data) { //
    ...
    if (sendSuggestionBtn) sendSuggestionBtn.classList.toggle('hidden', !isOwnProfile); // Oculta o botão se não for o próprio perfil //
...
function setupNotificationListener(uid) { //
    if (unsubscribeSuggestions) { //
        unsubscribeSuggestions(); // Desinscrever de qualquer listener anterior //
    }
...
async function loadRespondedSuggestions() { //
    ...
    respondedSuggestionsList.innerHTML = ''; // Limpa a lista antes de carregar //
    ...
    respondedSuggestions.sort((a, b) => { //
        ...
        if (aIsRead && !bIsRead) return 1; // b (não lida) vem antes de a (lida) //
        if (!aIsRead && bIsRead) return -1; // a (não lida) vem antes de b (lida) //
